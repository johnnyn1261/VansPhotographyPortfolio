...
export default function Navigation({ categories }: NavigationProps) {
  ...
  const renderNavLinks = (onLinkClick?: () => void) => (
    <nav className="flex flex-col gap-4 text-[10px] font-bold tracking-widest text-text-light uppercase">
      <Link
        href="/"
        onClick={onLinkClick}
    ...
    <>
      ...
      <aside className="hidden xs:flex w-44 shrink-0 border-r border-line-light bg-bg-base h-screen sticky top-0 flex-col px-6 py-8 z-30">
        ...
        <div className="flex flex-col gap-4 mt-auto pt-8 shrink-0">
          {renderSocialIcons()}
          <div className="text-[8px] tracking-widest text-text-light font-medium uppercase leading-relaxed">
            © {new Date().getFullYear()} Van-Nhan Nguyen<br />All rights reserved
          </div>
        </div>
      </aside>
